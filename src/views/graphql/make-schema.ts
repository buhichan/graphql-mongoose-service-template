    ...
    switch (true) {
        ...
        case field.type === "object" && "fields" in field && field.fields instanceof Array && field.fields.length > 0: {
            ...
            if (!context.outputTypeHashMap.has(ObjectTypeUniqueName))
                context.outputTypeHashMap.set(
                    ObjectTypeUniqueName,
                    new GraphQLObjectType({
                        name: ObjectTypeUniqueName,
                        description: field.label,
                        fields: () =>
                            (field as ObjectFieldMeta).fields.reduce((fields, childMeta) => {
                                const child = mapMetaToField(childMeta, context, path.concat(field.name))
                                if (child) fields[childMeta.name] = child
                                return fields
...
export function mapMetaToInputType(meta: IMeta, context: TypeMapperContext, path: string[], operationType: "Any" | "Read" | "Write"): GraphQLInputType | null {
    if (!meta) return null
    if (meta.resolve) return null //TBD: resolvable field is assumed readonly
    if ("readonly" in meta && operationType === "Write") return null
    if (meta.writeonly && operationType === "Read") return null
    if (meta.type === "ref") return GraphQLString
    if (meta.type === "object") {
        ...
        if (!context.inputTypeHashMap.has(inputObjectTypeUniqueName))
            context.inputTypeHashMap.set(
                inputObjectTypeUniqueName,
                new GraphQLInputObjectType({
                    name: inputObjectTypeUniqueName,
                    fields: () =>
                        (meta as ObjectFieldMeta).fields.reduce((inputFields, fieldMeta) => {
    ...
    if (isInputType(type)) return type
    else return null
}
...
const sortArgType = new GraphQLList(
    new GraphQLInputObjectType({
        name: "SortField",
        fields: {
            field: {
...
function convertSearchConditionToBson(search: any) {
    ...
    if (typeof search === "string" && /^[0-9a-f]{24}$/.test(search)) return new ObjectID(search)
    return search
}
...
export function makeGraphQLSchema(options: GraphqlPluginOptions) {
    let { connection, metas, mutations: mutationMetas = {}, queries: queryMetas = {} } = options
    options.metas.forEach(meta => {
        if (!validateData(meta, metaOfMeta)) throw new Error("Invalid meta: " + meta.name)
    })
    ...
    metas = metas
        .filter(x => x && x.type === "object")
        .map((modelMeta: ObjectFieldMeta) => {
            context.metaMap.set(modelMeta.name, modelMeta)
            return {
                ...modelMeta,
                fields: [...modelMeta.fields, ...internalFields.filter(x => !modelMeta.fields.some(y => y.name === x.name))],
            }
        })
    metas.forEach(meta => {
        context.batcherMap.set(
            meta.name,
            makeBatch((ids: string[]) => {
            ...
            resolve: async (_, args) => {
                ...
                if (!model) return []
                    ...
                    const query = model
                        ...
                        .skip(args.skip)
                    if (args.limit) return query.limit(args.limit)
        ...
        mutation: new GraphQLObjectType({
            ...
            fields: {
                ...metas.reduce((mutations, meta) => {
                    ...
                    if (!modelType) throw new Error("Cannot find modelType:" + meta.name)
                    ...
                    mutations[updateModelMutationName] = {
                        ...
                        resolve: async (source, args, context, info) => {
                            ...
                            const res = await model
                                .findByIdAndUpdate(args._id, args.payload, {
                                    new: true,
                                })
                                .exec()
                            return res
